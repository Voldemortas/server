...
const GLOBAL_PARAMS_69 = '{"h1":"dis is h1","text":"69"}'

describe.each(['h1', 'h2', 'numericParam/69'])('react test', (path) => {
  ...
  it(`has hash and global params`, async () => {
    ...
    const paramsToExpect =
      path === 'numericParam/69' ? GLOBAL_PARAMS_69 : GLOBAL_PARAMS
    expect(globalParams).toStrictEqual(paramsToExpect)
    expect(pagerContent).toContain(
      `<script type="module" src="/front/h1.js?hash=${hash}">`
    )
    expect(pagerContent).toContain(
      `<link rel="stylesheet" href="/front/h1.css?hash=${hash}" />`
    )
  })
